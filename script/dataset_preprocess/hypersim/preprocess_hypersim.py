...
if "__main__" == __name__:
    ...
    parser.add_argument(
        "--split_csv",
        type=str,
        default="/root/Marigold/script/dataset_preprocess/metadata_images_split_scene_v1.csv",
    )
    parser.add_argument("--dataset_dir", type=str, default="/root/Datasets/hypersim")
    parser.add_argument("--output_dir", type=str, default="/root/Dataset/Hypersim/precessed")
    ...
    for split in ["train", "val", "test"]:
        ...
        for i, row in tqdm(split_meta_df.iterrows(), total=len(split_meta_df)):
            ...
            # print("Checking path:", os.path.join(dataset_dir, dist_path))
            # print(f"[DEBUG] dataset_dir: {dataset_dir}, rgb_path: {rgb_path}, full_path: {os.path.join(dataset_dir, rgb_path)}")
